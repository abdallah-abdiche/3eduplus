...
// Get cart count from database if logged in
$cart_count = 0;
if ($is_logged_in) {
    $count_sql = "SELECT COUNT(*) as count FROM panier WHERE utilisateur_id = " . $_SESSION['user_id'];
...
if (isset($_POST['add_to_cart'])) {
    ...
    $price_query = "SELECT prix FROM formations WHERE formation_id = $formation_id";
    $price_res = $conn->query($price_query);
    $price = 0;
    if ($price_res && $price_res->num_rows > 0) {
        $price = $price_res->fetch_assoc()['prix'];
    }
    ...
    if ($stmt->execute()) {
        ...
        $count_res = $conn->query($count_sql);
        $new_count = $count_res->fetch_assoc()['count'];
